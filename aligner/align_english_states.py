...
def gen_res(tmpbase, outfile1, outfile2):
    ...
    while (i < len(lines)):
        if (len(lines[i].split()) >= 4) and (lines[i].split()[0] != lines[i].split()[1]):
            if (len(lines[i].split()) >= 6):
                phone = lines[i].split()[4]
            phn = phone + '_' + lines[i].split()[2]
            ...
            if phn == 'sp_s2' or lines[i].split()[2] == 's3':
                # print(frame, phone)
                phones_states_frame.append([frame, phone])
    ...
    with open(outfile2, 'w') as fwid:
        # for item in times2:
        #     fwid.write(str(item[1]) + ' ' + str(item[2]) + ' ' + item[0] + '\n')        
        for item in phones_states_frame:
            print(item[0], item[1])
...
if __name__ == '__main__':
    ...
    try:
        ...
        wavfile = '../input_audio/{person}/{file_name}.wav'.format(person=person, file_name=file_name)
        ...
        outfile2 = '../input_timestamp/{person}/phones_states/{file_name}.txt'.format(person=person, file_name=file_name)
    ...
    try:
        ...
        os.system(HVITE + ' -a -m -f -t 10000.0 10000.0 100000.0 -I ' + tmpbase + '.mlf -H ' + MODEL_DIR + '/16000/macros -H ' + MODEL_DIR + '/16000/hmmdefs -i ' + tmpbase +  '.aligned '  + tmpbase + '.dict ' + MODEL_DIR + '/monophones ' + tmpbase + '.plp 2>&1 > /dev/null') 
    except:
        print('HVite error!')
        sys.exit(1)
    ...
    #generate results
    try:
        print('Generating states results...')
